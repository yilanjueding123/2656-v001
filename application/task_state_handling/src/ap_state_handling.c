...
void ap_state_handling_connect_to_pc(INT32U prev_state)
{
	...
	#else
	OSTimeDly(50); //检测USB还是充电器插入时会init一次 USB（主要是检测PC传来的Reset信号），下面进入USBD的时候还会init一次，两次init间隔不能太近，否则USB硬体会有问题
	temp = 0;
	#endif
...
//	adc_user_line_in_en(ADC_LINE_3,0);
//fankun
#if 0
...
{		
 ...
 void ap_state_config_timefile_get(TIME_T *time)
 	{
 	  time->tm_year=file_time.tm_year;
	  time->tm_mon= file_time.tm_mon;
	  time->tm_mday= file_time.tm_mday;
	  time->tm_hour=file_time.tm_hour;
	  time->tm_min=file_time.tm_min;
	  time->tm_sec=file_time.tm_sec;
 	}
 
 void ap_state_config_timefile_set(TIME_T time)
 	{
 	 file_time.tm_year=time.tm_year;
	 file_time.tm_mon=time.tm_mon;
	 file_time.tm_mday=time.tm_mday;
	 file_time.tm_hour=time.tm_hour;
	 file_time.tm_min=time.tm_min;
	 file_time.tm_sec=time.tm_sec;
...
#if 1
void ap_state_handing_intime_init(TIME_T intime)
{
   INT32U time=0;
   time=3600*intime.tm_hour+60*intime.tm_min+intime.tm_sec;		//total seconds.
   rtc_ext_to_int_set_1(time); 
