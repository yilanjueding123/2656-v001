...
INT8S ap_video_record_func_key_active(INT32U event)
{
	...
	INT32U t;
	INT32U led_type;
DBG_PRINT("VIDEO_STS=%d\r\n",video_record_sts);
	video_down_flag=1;

	if (video_record_sts & VIDEO_RECORD_BUSY) {
	
		video_stop_flag=1;
		if(ap_video_save_state_get())
		{
			led_green_off();
			for(t=0; t<5; t++)
			{
		...
		if((event == MSG_APQ_VIDEO_RECORD_ACTIVE) || (event == MSG_APQ_CAPTUER_ACTIVE)) {
	...
	//	ap_state_handling_led_on();
		...
		///fankun1222
		//Card_exit=drvl2_sdc_live_response();
		//Card_exit=0;
		if(ap_state_handling_storage_id_get()!= NO_STORAGE)
			{			
...
//					ap_state_handling_icon_show_cmd(ICON_MOTION_DETECT, NULL, NULL);
			...
			#endif
		#else
		#if C_MOTION_DETECTION == CUSTOM_OFF
		INT64U  disk_free_size;
...
#if 1//C_AUTO_DEL_FILE == CUSTOM_ON
#if GPS_TXT
...
{
	MEDIA_SOURCE src;
    INT32U disk_free_size,led_type;

#if C_CYCLIC_VIDEO_RECORD == CUSTOM_ON
	INT32U time_interval, value;
	INT8U temp[9] = {0, 1, 2, 3, 5, 7, 10, 15, 20}; //7,10,15,   Liuxi modified in 2015-01-17,  
	...
	if (value == 0) {
        ...
        bkground_del_disable(1); //0 // back ground auto delete enable
        time_interval = value * VIDEO_RECORD_CYCLE_TIME_INTERVAL + 112;  // dominant add 64 to record more
    }
#endif
