...
// video 重送機制
static WebCamBuf vid_cur_buf, vid_dma_done;
static WebCamBuf vid_buf[UVC_SIZE];
INT32U vid_buf_addr[UVC_SIZE]; //add
static int vid_wi;
static int vid_ri;
...
static int set_vid_idx(INT8U *addr , INT32U size)
{
	...
	if (idx==vid_ri)
	{
		DBG_PRINT("VERR! %d_%d\r\n",vid_wi,vid_ri);
		// drop this frame
		vid_dma_done.addr = addr;
		vid_dma_done.size = size;
		OSQPost(my_AVIEncodeApQ, (void*)MSG_UVC_BUF_DMA_DOWN);		
		return -1;
	}
	else
	{
...
{
	vid_dma_done.addr = vid_cur_buf.addr;
...
unsigned int video_done_packet(void)
{
	return (unsigned int)(vid_dma_done.addr);
}
