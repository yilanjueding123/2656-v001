...
ALARM_T alarm_event[ALARM_MAX_NUM] =
{
	0
};
...
int timezone_adj[] =
{
	0, - (12 * 60 + 00) 							/* 1(GMT-12:00) Eniwetok,Kwajalain
												 */
	...
	, 0 											/* 22(GMT)		 Casablance, Monrovia
											   */
	, 0 											/* 23(GMT)		 Greenwich Mean Time:Dublin,Edinburgh,Lisbon,
		London		   */
...
void Time_card_storage(TIME_T * intime)
{
	...
	if ((intime->tm_mon > 12 || intime->tm_mon < 1) || (intime->tm_mday > monday[intime->tm_mon] ||
		 intime->tm_mday <1)||
	(intime->tm_hour > 23 || intime->tm_hour < 0) || (intime->tm_min > 59 || intime->tm_min < 0) ||
		 (intime->tm_sec > 59 || intime->tm_sec < 0) || (intime->tm_year < 2014))
#endif
	...
	j					= date_to_jd(intime->tm_year, intime->tm_mon, intime->tm_mday);
	c					= intime->tm_sec + intime->tm_min * 60 + intime->tm_hour * 3600;
...
INT32S cal_time_set(TIME_T tm)
{
	...
	if ((tm.tm_mon > 12 || tm.tm_mon < 1) || (tm.tm_mday > monday[tm.tm_mon] || tm.tm_mday < 1) || (tm.tm_hour > 23 ||
		 tm.tm_hour < 0) || (tm.tm_min > 59 || tm.tm_min < 0) || (tm.tm_sec > 59 || tm.tm_sec < 0) ||
		 (tm.tm_year < 1800))
	{
		return STATUS_FAIL;
	}
...
static INT32S set_next_alarm(void)
{
	...
	for (i = 0; i < ALARM_MAX_NUM; i++)
	{
		...
		{
			temp[count] 		=
				 alarm_event[i].alarm_time.tm_sec + alarm_event[i].alarm_time.tm_min * 60 + alarm_event[i].alarm_time.tm_hour * 3600;
			count++;
		}
	}
