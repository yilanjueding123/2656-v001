...
void Time_card_storage(TIME_T *intime)
{
      ...
      t_rtc  r_time;	   
		 ...
		 if( (intime->tm_mon > 12 ||  intime->tm_mon < 1)||(intime->tm_mday > monday[intime->tm_mon] || intime->tm_mday<1)||
		 	 (intime->tm_hour >23 || intime->tm_hour < 0)||(intime->tm_min >59||intime->tm_min < 0)||(intime->tm_sec >59||intime->tm_sec<0)
		 	 ||( intime->tm_year <2014))
		 	 #endif
		 	{
		 	 intime->tm_year=2014;
